...
class SaveWindows:
    def __init__(self, params):
        ...
        self.pc_0_dir = glob.glob(str(params['data_dir']) + f'/*t{params["pc_0"]}.*', recursive=True)[0]
        self.pc_1_dir = glob.glob(str(params['data_dir']) + f'/*t{params["pc_1"]}.*', recursive=True)[0]
        os.makedirs(f'{self.out_path}/pc{self.pc_0_dir[-5]}', exist_ok=True)  # create pc_0 folder
        os.makedirs(f'{self.out_path}/pc{self.pc_1_dir[-5]}', exist_ok=True)  # create pc_1 folder
        self.tagging_method = params['tagging_method']
        self.load_dataset(self.pc_0_dir)  # load dataset into self.dataset
...
def generate_windows(params, num_workers=32):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=num_workers) as executor:
        ...
        for i in executor.map(sw.save_window, coords_pairs):
            pc_0_filenames.append(i[0])
            pc_0_y_label.append(i[1])
    #  load pc_1 dataset
    sw.load_dataset(sw.pc_1_dir)
    #  run pc_1
    with concurrent.futures.ThreadPoolExecutor(max_workers=num_workers) as executor:
        print('starting threads')
        pc_1_filenames = []
    ...
    sw.save_preprocess_metadata(pd.Series(pc_0_filenames), pd.Series(pc_1_filenames), pd.Series(pc_0_y_label))
